...
class BPRegressionModel(nn.Module):
    def __init__(self):
        ...
        # Fix input size for LSTM: 32 channels * 16 * 16 = 8192
    ...
    def forward(self, x, bpm):
        ...
        # Reshape for 3D CNN: (batch_size, channels, frames, height, width)
        # Your load_video_frames already returns (1, 100, 64, 64)
        # We need to add the channel dimension: (1, 1, 100, 64, 64)
        x = x.unsqueeze(1) # Add channel dimension
        
        # We need to process frame-by-frame for the CNN part if it's 3D
        # Your CNN is 3D, so (batch, channel, frames, h, w) is correct.
        
        cnn_out = self.cnn(x)
        
        # Calculate the flattened size dynamically
        # After two MaxPool3d(2,2,2):
        # Frames: 100 -> 50 -> 25
        # H/W: 64 -> 32 -> 16
        # Size: 32 * 25 * 16 * 16
        # Let's reshape cnn_out to (batch_size, frames, features) for LSTM
        
        # Re-check dimensions after CNN
        # Input (1, 1, 100, 64, 64)
        # Conv1 (1, 16, 100, 64, 64) -> MaxPool1 (1, 16, 50, 32, 32)
        # Conv2 (1, 32, 50, 32, 32) -> MaxPool2 (1, 32, 25, 16, 16)
        # Flatten: (1, 32 * 25 * 16 * 16) - This doesn't seem right for LSTM
        
        # Let's assume your original model expected (batch_size, frames, features)
        # Let's re-implement the flatten and LSTM part
        
        cnn_out = self.cnn(x) # Output: (1, 32, 25, 16, 16)
        ...
        # Your LSTM input_size is 32 * 16 * 16 = 8192
        # My calculation is 32 * 16 * 16 = 8192. Perfect.
...
# The if __name__ == "__main__": block is removed as it's not needed